...
Отчётные таблицы хранят показатели по дням. Недельные, месячные и годовые значения рассчитываются из них на уровне отчёта.
...
`player_metrics` строится после независимой агрегации каждого факта до уровня игрока и дня. `metrics_overview` агрегирует показатели `player_metrics` по стране и типу регистрации. Прямое соединение трёх таблиц фактов не используется, поскольку оно размножает строки и искажает суммы.
